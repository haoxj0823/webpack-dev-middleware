...
module.exports = function wrapper(context) {
  return function middleware(req, res, next) {
    ...
    if (basename === 'index') {
      ...
      compiler.options.plugins = compiler.options.plugins.filter(p => !(p instanceof HtmlWebpackPlugin))

      const distPath = normalPath(compiler.options.output.path);
      ...
      let tsFile = normalPath(`${dirname}/${basename}.ts`);

      compiler.options.entry[chunkName] = [`.${path.sep}${path.relative(rootPath, tsFile)}`];

      let headFileName = path.resolve(dirname, 'head.html');
      if (fs.existsSync(headFileName)) {
        compiler.options.entry[chunkName].push(`.${path.sep}${path.relative(rootPath, headFileName)}`);
      }

      let bodyFileName = path.resolve(dirname, basename + '.html');
      if (fs.existsSync(bodyFileName)) {
        compiler.options.entry[chunkName].push(`.${path.sep}${path.relative(rootPath, bodyFileName)}`);
      }

      let styleFileName = path.resolve(dirname, basename + '.less');
      if (fs.existsSync(styleFileName)) {
        compiler.options.entry[chunkName].push(`.${path.sep}${path.relative(rootPath, styleFileName)}`);
      }
      ...
      new MultiEntryPlugin(compiler.options.context, compiler.options.entry[chunkName], chunkName).apply(compiler);
      new HtmlWebpackPlugin(htmlwebpackpluginConf).apply(compiler);
